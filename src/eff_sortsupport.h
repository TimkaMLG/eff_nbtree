 ...
 * provide a BTEffSortSupport_PROC pg_amproc entry, which must take a single
 * argument of type internal and return void.  The argument is actually a
 * pointer to a EffSortSupportData struct, which is defined below.
 *
 * If provided, the BTEffSortSupport function will be called during sort setup,
 ...
 * required to provide all of them.  The BTEffSortSupport function should
 * simply not set any function pointers for mechanisms it doesn't support.
 * Opclasses that provide BTEffSortSupport and don't provide a comparator
 ...
 * EffSortSupportData struct when called, so they can use it to store
 * additional private data as needed.  In particular, for collation-aware
 * datatypes, the ssup_collation field is set before calling BTEffSortSupport
 ...
 * it is possible to associate a BTEffSortSupport function with a cross-type
 ...
 *
 * src/include/utils/EffSortSupport.h
 *
 *-------------------------------------------------------------------------
 */
#ifndef EFFSortSupport_H
#define EFFSortSupport_H

#include "access/attnum.h"
#include "utils/relcache.h"

typedef struct EffSortSupportData *EffSortSupport;

typedef struct EffSortSupportData
{
	/*
	 * These fields are initialized before calling the BTEffSortSupport function
	 ...
	 * changed after BTEffSortSupport is called, so don't use them in selecting
	...
	/*
	 * ssup_extra is zeroed before calling the BTEffSortSupport function, and is
	 * not touched subsequently by callers.
	 */
	void	   *ssup_extra;		/* Workspace for opclass functions */

	/*
	 * Function pointers are zeroed before calling the BTEffSortSupport function,
	 ...
	 */
	int			(*comparator) (Datum x, Datum y, EffSortSupport ssup);

	/*
	 * "Abbreviated key" infrastructure follows.
	 *
	 * All callbacks must be set by EffSortSupport opclasses that make use of
	 ...
	 * EffApplySortAbbrevFullComparator() is therefore required.  In many cases
	 ...
	 * is applicable in principle (that is, the EffSortSupport routine needs to
	 ...
	 * (which is something EffSortSupport routines need not concern themselves
	 * with). However, EffSortSupport routines must not set it when it is
	 ...
	 * abbreviation).
	 */
	Datum		(*abbrev_converter) (Datum original, EffSortSupport ssup);

	/*
	 * abbrev_abort callback allows clients to verify that the current
	 * strategy is working out, using a EffSortSupport routine defined ad-hoc
	 ...
	 */
	bool		(*abbrev_abort) (int memtupcount, EffSortSupport ssup);
	 ...
	 * was inconclusive (by calling EffApplySortAbbrevFullComparator()), or used
	 * to replace "comparator" when core system ultimately decides against
	 * abbreviation.
	 */
	int			(*abbrev_full_comparator) (Datum x, Datum y, EffSortSupport ssup);
} EffSortSupportData;


/*
 * EffApply a sort comparator function and return a 3-way comparison result.
 * This takes care of handling reverse-sort and NULLs-ordering properly.
 */
static inline int
EffApplySortComparator(Datum datum1, bool isNull1,
...
static inline int
EffApplyUnsignedSortComparator(Datum datum1, bool isNull1,
							Datum datum2, bool isNull2,
							EffSortSupport ssup)
{
	int			compare;
...
#if SIZEOF_DATUM >= 8
static inline int
EffApplySignedSortComparator(Datum datum1, bool isNull1,
						  Datum datum2, bool isNull2,
						  EffSortSupport ssup)
{
	int			compare;
...
static inline int
EffApplyInt32SortComparator(Datum datum1, bool isNull1,
						 Datum datum2, bool isNull2,
						 EffSortSupport ssup)
{
	int			compare;
...
/*
 * EffApply a sort comparator function and return a 3-way comparison using full,
 * authoritative comparator.  This takes care of handling reverse-sort and
 * NULLs-ordering properly.
 */
static inline int
EffApplySortAbbrevFullComparator(Datum datum1, bool isNull1,
							  Datum datum2, bool isNull2,
							  EffSortSupport ssup)
{
	int			compare;
 ...
 */
extern int	eff_ssup_datum_unsigned_cmp(Datum x, Datum y, EffSortSupport ssup);
#if SIZEOF_DATUM >= 8
extern int	eff_ssup_datum_signed_cmp(Datum x, Datum y, EffSortSupport ssup);
#endif
extern int	eff_ssup_datum_int32_cmp(Datum x, Datum y, EffSortSupport ssup);

/* Other functions in utils/sort/EffSortSupport.c */
extern void PrepareEffSortSupportComparisonShim(Oid cmpFunc, EffSortSupport ssup);
extern void PrepareEffSortSupportFromOrderingOp(Oid orderingOp, EffSortSupport ssup);
extern void PrepareEffSortSupportFromIndexRel(Relation indexRel, int16 strategy,
										   EffSortSupport ssup);
extern void PrepareEffSortSupportFromGistIndexRel(Relation indexRel, EffSortSupport ssup);

#endif							/* EffSortSupport_H */
